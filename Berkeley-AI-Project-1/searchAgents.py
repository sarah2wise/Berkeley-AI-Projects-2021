...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.corners_left = [False, False, False, False] # initial corner state
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0,4):
        if state[1][i] == False:
            ...
            if h > bigh:
                bigh = h
    return bigh # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        #h = abs(position[0] - food[0]) + abs(position[1] - food[1])
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # just copied my BFS implementation here
        frontier = util.Queue()
        expanded = []
        path = []
        frontier.push((startPosition,path))
        while not frontier.isEmpty():
            ...
            if state not in expanded:
                ...
                for child_state in problem.expand(state):
                   # child_state[0] = next state
                   # child_state[1] = action
                    new_path = path + [child_state[1]]
                    frontier.push((child_state[0],new_path))
        return path
